            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        ...
                        <button
                            @click="copy('{{ $selectedProduct->product_id }}', 'Product ID')"
                ...
                <div class="flex items-center gap-2">
                    @php
                        $margin = $selectedProduct->retail_price > 0
                            ? round((($selectedProduct->retail_price - $selectedProduct->purchase_price) / $selectedProduct->retail_price) * 100, 1)
                            : 0;
                    @endphp
                ...
                <div class="md:col-span-2 space-y-6">
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button @click="copy('{{ $selectedProduct->description ?? '' }}', 'Description')" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 transition-colors">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button @click="copy('{{ $selectedProduct->purchase_price }}', 'Cost Price')" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 transition-colors">
                                    <flux:icon.copy class="size-4 cursor-pointer" />
                                </button>
                            </div>
                            <flux:input readonly value="Rs. {{ number_format($selectedProduct->purchase_price, 0) }}" icon="banknotes" />
                        </div>

                        <div>
                            <div class="flex items-center justify-between mb-2">
                                <flux:label>Retail Price</flux:label>
                                <button @click="copy('{{ $selectedProduct->retail_price }}', 'Retail Price')" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 transition-colors">
                                    <flux:icon.copy class="size-4 cursor-pointer" />
                                </button>
                            </div>
                            <!-- Prominent Retail Price -->
                            <div class="relative">
                                <flux:input readonly value="Rs. {{ number_format($selectedProduct->retail_price, 0) }}" icon="tag" class="font-bold text-lg text-blue-600 dark:text-blue-400" />
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button @click="copy('{{ $selectedProduct->delivery_charges }}', 'Delivery Charges')" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 transition-colors">
                                    <flux:icon.copy class="size-4 cursor-pointer" />
                                </button>
                            </div>
                            <flux:input readonly value="Rs. {{ number_format($selectedProduct->delivery_charges,0) }}" icon="truck" />
                        </div>
                        @endif
                    </div>
                </div>
