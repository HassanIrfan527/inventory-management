...
<div {{ $attributes->merge(['class' => 'group relative flex items-start gap-3 rounded-lg p-3 transition hover:bg-zinc-50 dark:hover:bg-zinc-800/30']) }}>
    @if ($icon)
        <flux:icon :name="$icon" class="mt-0.5 h-5 w-5 flex-shrink-0 text-zinc-400" />
    @endif
    
    <div class="flex-1 min-w-0">
        <p class="text-xs font-medium uppercase tracking-wide text-zinc-500 dark:text-zinc-400 mb-1">{{ $label }}</p>
        ...
        @else
            ...
            @else
                <p class="text-sm font-medium {{ $value ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-500' }}">
                    {{ $value ?: $emptyText }}
                </p>
            @endif
        @endif
